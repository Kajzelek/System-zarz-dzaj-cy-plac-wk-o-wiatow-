...
public class UwagaDAOImpl implements UwagaDAO {
    ...
    public List<Uwaga> findUwagiWystawioneByIdNauczyciela(int id) {
        TypedQuery<Uwaga> query = entityManager.createQuery("from Uwaga where wystawiajacy.id = :data", Uwaga.class);
    ...
    public List<Uwaga> findUwagiWystawioneByIdUcznia(int id) {
        TypedQuery<Uwaga> query = entityManager.createQuery("from Uwaga where otrzymujacy.id = :data", Uwaga.class);
        query.setParameter("data", id);
        return query.getResultList();
    }
}
